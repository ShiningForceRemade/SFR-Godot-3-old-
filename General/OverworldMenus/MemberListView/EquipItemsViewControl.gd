	...
	for item in Singleton_Game_GlobalCommonVariables.selected_character.inventory:
		...
		if irl is CN_SF1_Item_Weapon:
			...
			for class_req in irl.equippable_by:
				if class_req == Singleton_Game_GlobalCommonVariables.selected_character.class_idx:
...
func _process(_delta):
	...
	if Input.is_action_just_pressed("ui_a_key"):
		if selected_item == E_SelectedItem.DOWN:
			for i in Singleton_Game_GlobalCommonVariables.selected_character.inventory.size():
			...
			for i in Singleton_Game_GlobalCommonVariables.selected_character.inventory.size():
				...
				if item is CN_SF1_Item_Weapon:
					if item.item_name == war.item_name:
						Singleton_Game_GlobalCommonVariables.selected_character.inventory[i].is_equipped = true
					else:
						Singleton_Game_GlobalCommonVariables.selected_character.inventory[i].is_equipped = false
...
func DisplayCharacterStats(force_member) -> void: 
	...
	var i = 0
	for item in Singleton_Game_GlobalCommonVariables.selected_character.inventory:
		print(Singleton_Game_GlobalCommonVariables.selected_character.inventory[i].is_equipped)
		if Singleton_Game_GlobalCommonVariables.selected_character.inventory[i].is_equipped == false:
			i = i + 1
			continue
...
func PreviewDisplayCharacterStatsWithNewEquipSelection(weapon_resource_preview) -> void:
	var c = Singleton_Game_GlobalCommonVariables.selected_character

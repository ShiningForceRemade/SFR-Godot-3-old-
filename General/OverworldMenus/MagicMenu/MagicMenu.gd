...
func set_battle_magic_menu_active() -> void:
	...
	print("Magic Menu Current Char", Singleton_Game_GlobalCommonVariables.main_character_player_node.characterRoot)
	print("Magic Menu Spells", Singleton_Game_GlobalCommonVariables.main_character_player_node.characterRoot.spells_id)
	character_spells = Singleton_Game_GlobalCommonVariables.main_character_player_node.characterRoot.spells_id
...
#			get_parent().get_node("BattleActionsMenuRoot").set_menu_active()
		...
		if event.is_action_released("ui_a_key"): # event.is_action_released("ui_accept"):
			...
			var actor = Singleton_Game_GlobalCommonVariables.main_character_player_node.characterRoot
	...
	if is_select_magic_level_active:
		...
		if event.is_action_released("ui_a_key"): #event.is_action_released("ui_accept"):
			...
			if Singleton_Game_GlobalCommonVariables.main_character_player_node.characterRoot.cget_mp_current() < actor.spells_id[currently_selected_option].levels[0].mp_usage_cost:
